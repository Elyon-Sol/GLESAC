...
> and the project is **open source under AGPL-3.0** (same license as the Elyon-Sol core). The design
> substance below (local-first, read-only console, SoD, whitelisted runbooks) still describes the code.

Design of record for Gargoyles Ledge (the Elyon-Sol Administrative Console). This document is
self-contained and lives in this repo. GLESAC is an AGPL-3.0 operator tooling layer that consumes
Elyon-Sol as an external dependency, by invocation, never by importing core internals.
